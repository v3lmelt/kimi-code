...
Hasu CLI 的 TUI 交互模式支持一套键盘快捷键。键位按使用场景分为五组：通用输入、模式切换、流式输出期间、工具输出控制、审批面板，以及弹窗浏览。在 TUI 中输入 `/help` 可随时打开内置快捷键清单。
...
按 `Shift-Tab` 可在模式间循环切换：Plan → Auto → YOLO → 默认（manual）模式。Plan 模式下 Agent 优先使用只读工具进行研究和规划，并可写入当前计划文件；`Bash` 按当前权限模式和普通规则处理，不会因 Plan 模式额外发起独立审批。Auto 模式自动批准所有操作；YOLO 模式自动批准常规工具调用，但在敏感操作前仍会询问。各模式互斥：进入 Auto 或 YOLO 会自动关闭 Plan 模式，进入 Plan 模式会将权限重置为 manual。单纯循环切换不会创建空计划文件。
...
- [hasu 命令](./kimi-command.md) — 启动参数与子命令完整参考

...
Hasu CLI includes three built-in sub-agents, ready to use out of the box, each aimed at a different task shape:
...
Hasu CLI discovers agent files by scope; more specific scopes take higher priority: **Explicit (`--agent-file`) > Project > Extra > User > Plugin > Built-in**. When two files define the same `name`, the higher-priority scope wins. Each directory is scanned recursively for `.md` files.

**User level** (applies to all projects):
- `$KIMI_CODE_HOME/agents/` (default: `~/.kimi-code/agents/`)
- `~/.agents/agents/`

The Hasu-specific user agent directory moves with `KIMI_CODE_HOME`, while the generic `~/.agents/agents/` directory stays under the real OS home so it can be shared across tools.
...
| `model_preference` | no | Default model used when `Agent` or `AgentSwarm` spawns this profile: `primary` selects the model the caller is currently running, `secondary` selects [`[secondary_model] model`](../configuration/config-files.md#secondary-model), `inherit` explicitly inherits the caller's model, and any other value is taken as a concrete model alias configured in `[models]`. An explicit tool-call `model` (which likewise accepts any of these) wins over this field; without either, the configured secondary model remains the default, otherwise the subagent inherits the caller's model. The reserved values `primary` / `secondary` / `inherit` shadow same-named `[models]` aliases |
...
`model_preference` applies only to newly spawned subagents. It takes effect in every launch mode, including the interactive TUI. A concrete model alias is validated against `[models]` at spawn time; an unknown alias fails the spawn with an actionable error. Resumed subagents keep their existing model. The selected preference is shown to the main agent alongside the profile description so it can still pass an explicit `model` when a task needs a different choice.
...
Two CLI flags select which agent drives a new session, in both print mode (`hasu -p`) and the interactive TUI:
...
```markdown
You are Hasu, running at ${cwd} on ${os}.

...
When resuming a session, you can override its saved permission or plan mode by adding `--auto`, `--yolo`, or `--plan`. For example, `hasu --continue --auto` resumes the latest session and switches it to auto permission mode.
...
- **`--skills-dir <dir>`** (CLI flag): **Replaces** the automatically discovered user and project directories for this launch only. Can be repeated to stack multiple directories:
  ...
  ```sh
  hasu --skills-dir /path/to/team-skills --skills-dir ./local-skills
  ```
...
`--agent` and `--agent-file` select which agent drives a new session, in both print mode (`hasu -p`) and the interactive TUI:

```sh
hasu --agent reviewer
hasu -p --agent reviewer "Review the changes on this branch"
```
...
```sh
hasu -p "Summarize the current repository status"
```
...
`hasu` provides the following subcommands: `login` (non-interactive login), `acp` (ACP IDE mode), `doctor` (validate configuration files), `export` (export a session), `migrate` (migrate legacy data), `upgrade` (check for updates), and `provider` (manage providers).
...
Switch Hasu CLI to ACP (Agent Client Protocol) mode, communicating with an IDE via JSON-RPC over stdin/stdout so the editor can directly drive hasu's sessions and tool calls. You typically do not need to run this manually — the IDE starts it as a subprocess entry point. For configuration, see [Using in IDEs](../guides/ides.md); for technical details, see the [hasu acp reference](./kimi-acp.md).
...
| Command | Description |
| --- | --- |
| `hasu doctor` | Validate the default `config.toml` and `tui.toml` |
| `hasu doctor config [path]` | Validate only `config.toml`, using `path` instead of the default file when provided |
| `hasu doctor tui [path]` | Validate only `tui.toml`, using `path` instead of the default file when provided |
...
hasu export 01HZ...XYZ -o ./bug-report.zip

# Exclude the global diagnostic log
hasu export 01HZ...XYZ -o ./bug-report.zip --no-include-global-log
```
...
Immediately check for the latest version and display an update prompt; exits after you make a selection. `hasu update` is an alias for this command.

```sh
hasu upgrade
```

For global npm, pnpm, yarn, bun, and macOS / Linux native installations, `hasu upgrade` shows update options; selecting `Install update now` runs the corresponding foreground install command. When the current installation method cannot be upgraded automatically (e.g., Windows native installation), the manual update command is printed instead.
...
hasu vis --host 0.0.0.0 --port 8123 --no-open
```
...
```sh
hasu provider <action> [options]
```

Five actions are available:

#### `hasu provider add <url>`
...
```sh
hasu provider add https://registry.example.com/v1/models/api.json --api-key YOUR_KEY

# Or via environment variable (suitable for CI / .envrc)
KIMI_REGISTRY_API_KEY=YOUR_KEY hasu provider add https://registry.example.com/v1/models/api.json
```
...
#### `hasu provider remove <providerId>`
...
```sh
hasu provider list
hasu provider list --json | jq '.providers | keys'
```

#### `hasu provider catalog list [providerId]`
...
```sh
hasu provider catalog list
hasu provider catalog list --filter anthropic
hasu provider catalog list anthropic
```

#### `hasu provider catalog add <providerId>`
...
```sh
hasu provider catalog list anthropic          # Browse available models first
hasu provider catalog add anthropic --api-key sk-ant-... --default-model claude-opus-4-7
```

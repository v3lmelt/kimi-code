...
`hasu acp` switches Hasu CLI to **ACP (Agent Client Protocol)** mode: it communicates with an ACP client (such as Zed, JetBrains AI Chat, etc.) via JSON-RPC over stdin/stdout, letting the IDE directly drive hasu's sessions, prompts, and tool calls.
...
::: tip Who calls this?
You typically do not need to run `hasu acp` manually — this command is the subprocess entry point for IDEs. For IDE-side configuration, see [Using in IDEs](../guides/ides.md).
:::
...
| --- | --- | --- |
| `initialize` | Yes | Version negotiation; returns `agentInfo: { name: 'Hasu CLI', version }`, capability matrix, and `authMethods` |
...
- `http` → hasu's `transport: 'http'` configuration
- `stdio` → hasu's `transport: 'stdio'` configuration
- `sse` → hasu's `transport: 'sse'` configuration
- `acp` → discarded with a warn log entry

## Next steps

- [Using in IDEs](../guides/ides.md) — Zed / JetBrains configuration steps and troubleshooting
- [`hasu` Command Reference](./kimi-command.md) — Complete subcommand list

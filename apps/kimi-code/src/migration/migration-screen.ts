...
export interface MigrationScreenOptions {
   ...
   * now/later/never gate — used by the explicit `hasu migrate` command, where
...
export class MigrationScreenComponent extends Container implements Focusable {
  ...
  constructor(opts: MigrationScreenOptions) {
    ...
    if (opts.skipDecisionStep === true) {
      // Explicit `hasu migrate`: the now/later/never gate is meaningless, so
  ...
  private renderResult(width: number): string[] {
    ...
    if (this.migrationFailed) {
      ...
      lines.push('');
      lines.push(chalk.hex(colors.text)(' You can retry later by running "hasu migrate".'));
      lines.push('');
      lines.push(chalk.hex(colors.textMuted)(' ⏎ continue to hasu'));
...
function stepFor(phase: Phase, plan: MigrationPlan): StepDef {
  if (phase === 'ask1') {
    return {
      title: 'Migrate this data to hasu?',
      options: [

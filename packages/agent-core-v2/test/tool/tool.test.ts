...
describe('SubagentToolInputSchema', () => {
  ...
  it('exposes a free-form model choice parameter in the JSON schema', () => {
    const properties = agentSchemaProperties<{
      description?: string;
      type?: string;
      enum?: unknown;
    ...
    // Any configured model alias is accepted, so the schema is a bare string.
    ...
    expect(modelDescription).toContain('secondary');
    expect(modelDescription).toContain('primary');
    expect(modelDescription).toContain('inherit');
  });
...
describe('Agent tool description', () => {
  ...
  it('lists the primary model in the models section by default', () => {
    ctx = createTestAgent();
  ...
  it('advertises the model parameter when the experiment is enabled', () => {
    ...
    const properties = agentParameters()['properties'] as Record<string, { type?: string; enum?: unknown }>;
...
describe('AgentSwarm tool description', () => {
  ...
  it('advertises the model parameter when the experiment is enabled', () => {
    ...
    const properties = agentSwarmParameters()['properties'] as Record<string, { type?: string; enum?: unknown }>;
...
describe('Agent tools', () => {
  ...
  describe('foreground Agent tool recovery', () => {
    ...
    it('resolves with a partial summary when the final summary is truncated', async () => {
      await ctx.dispose();
      ctx = createTestAgent();
      ...
      // Truncation is self-healed by the loop's bounded recovery chain; when a
      // turn still settles truncated every recovery cap is exhausted, so the
      // partial output is accepted as a best-effort completion instead of failing.
      await expect(run.completion).resolves.toMatchObject({
        summary: expect.stringContaining('partial summary'),
      });
    });
  });
  ...
  describe('registered user tools', () => {
    ...
    it('routes registered user tools through tool.call request/response', async () => {
      ...
      expect(ctx.lastLlmInput()).toMatchInlineSnapshot(`
        system: <system-prompt>
        tools: Agent, AgentSwarm, AskUserQuestion, Bash, CreateGoal, Edit, EnterPlanMode, ExitPlanMode, FetchURL, GetGoal, Glob, Grep, Lookup, Read, SetGoalBudget, Skill, TaskList, TaskOutput, TaskStop, TodoList, UpdateGoal, Workflow, Write
        messages:
      ...
      expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
        ...
        [emit] tool.result                 { "turnId": 0, "toolCallId": "call_lookup", "uiPreview": "moon-result", "truncated": false, "output": "moon-result" }
        [emit] agent.activity.updated      { "lifecycle": "ready", "turn": { "turnId": 0, "origin": { "kind": "user" }, "phase": "running", "step": 1, "ending": false, "pendingApprovals": [], "activeToolCalls": [], "since": "<time>" }, "background": [] }
        ...
        [wire] llm.request                 { "kind": "loop", "provider": "openai", "model": "mock-model", "modelAlias": "mock-model", "thinkingEffort": "off", "maxTokens": 1000000, "toolSelect": false, "systemPromptHash": "ec9c34379c88babbc468ef2f3e0e08cd2f422c8c4a910664fb8bb394d703a575", "toolsHash": "75e580407c8908260be5e5252003b6acf4e8a9f029bf299eb9fe01a261745207", "messageCount": 4, "turnStep": "0.2", "time": "<time>" }
    ...
    `);
      ...
      expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
        ...
        [emit] context.spliced              { "start": 6, "deleteCount": 0, "messages": [ { "role": "user", "content": [ { "type": "text", "text": "<system-reminder>\\n<total_tokens>999824 tokens left</total_tokens>\\n</system-reminder>" } ], "toolCalls": [], "origin": { "kind": "injection", "variant": "total_tokens" } } ] }
        [wire] context.append_message       { "message": { "role": "user", "content": [ { "type": "text", "text": "<system-reminder>\\n<total_tokens>999824 tokens left</total_tokens>\\n</system-reminder>" } ], "toolCalls": [], "origin": { "kind": "injection", "variant": "total_tokens" } }, "time": "<time>" }
        ...
        [wire] usage.record                 { "model": "mock-model", "usage": { "inputOther": 206, "output": 10, "inputCacheRead": 0, "inputCacheCreation": 0 }, "usageScope": "turn", "time": "<time>" }
        [emit] agent.status.updated         { "usage": { "byModel": { "mock-model": { "inputOther": 514, "output": 38, "inputCacheRead": 0, "inputCacheCreation": 0 } }, "total": { "inputOther": 514, "output": 38, "inputCacheRead": 0, "inputCacheCreation": 0 }, "currentTurn": { "inputOther": 206, "output": 10, "inputCacheRead": 0, "inputCacheCreation": 0 } } }
        [emit] agent.status.updated         { "contextTokens": 216 }
        [emit] turn.step.completed          { "turnId": 1, "step": 1, "stepId": "<uuid-6>", "usage": { "inputOther": 206, "output": 10, "inputCacheRead": 0, "inputCacheCreation": 0 }, "finishReason": "end_turn", "providerFinishReason": "completed", "rawFinishReason": "stop" }
        [emit] agent.activity.updated       { "lifecycle": "ready", "turn": { "turnId": 1, "origin": { "kind": "user" }, "phase": "running", "step": 1, "ending": false, "pendingApprovals": [], "activeToolCalls": [], "since": "<time>" }, "background": [] }
        ...
        [wire] context.append_loop_event    { "event": { "type": "step.end", "uuid": "<uuid-6>", "turnId": "1", "step": 1, "finishReason": "end_turn", "usage": { "inputOther": 206, "output": 10, "inputCacheRead": 0, "inputCacheCreation": 0 }, "messageId": "mock-3", "providerFinishReason": "completed", "rawFinishReason": "stop" }, "time": "<time>" }
        [wire] turn.ended                   { "turnId": 1, "reason": "completed", "time": "<time>" }
        [emit] turn.ended                   { "turnId": 1, "reason": "completed" }
      `);
      expect(ctx.lastLlmInput()).toMatchInlineSnapshot(`
        tools: Agent, AgentSwarm, AskUserQuestion, Bash, CreateGoal, Edit, EnterPlanMode, ExitPlanMode, FetchURL, GetGoal, Glob, Grep, Read, SetGoalBudget, Skill, TaskList, TaskOutput, TaskStop, TodoList, UpdateGoal, Workflow, Write
        messages:
          <last>

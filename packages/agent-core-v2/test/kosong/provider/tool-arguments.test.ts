/**
 * `kosong/provider` tool-arguments parse-cache probes — mirror of
 * `packages/kosong/test/providers/tool-arguments.test.ts` (the kosong fork
 * contract): byte-stable tool-call argument strings parse once and are reused
 * across requests, different strings never interfere, failures re-run with the
 * exact original error types, and returned objects are detached shallow copies
 * so callers cannot poison the cache.
 */

import { ChatProviderError } from '#/kosong/contract/errors';
import { parseToolCallArguments } from '#/kosong/provider/bases/tool-arguments';

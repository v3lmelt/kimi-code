...
describe('Agent loop', () => {
  ...
  it('emits hosted search lifecycle, action, source, and completed events', async () => {
    const source = { type: 'url' as const, url: 'https://nodejs.org/', title: 'Node.js' };
    const action = { type: 'search' as const, query: 'current Node.js LTS', sources: [source] };
    ctx.mockNextProviderResponse({
      parts: [
        { type: 'hosted_search_lifecycle', callId: 'search-1', status: 'in_progress' },
        { type: 'hosted_search_action', callId: 'search-1', action },
        { type: 'hosted_search_source', callId: 'search-1', source },
        { type: 'text', text: 'Node.js is current.' },
      ],
      searchMetadata: [{ callId: 'search-1', status: 'completed', action, sources: [source] }],
      annotations: [
        {
          type: 'url_citation',
          startIndex: 0,
    ...
    await ctx.rpc.prompt({ input: [{ type: 'text', text: 'Search Node.js' }] });
    const events = await ctx.untilTurnEnd();
    const hosted = (events as unknown as Array<{ event: string; args: unknown }>)
      .filter((event) => event.event === 'hosted.search')
      .map((event) => event.args);

    expect(hosted).toMatchObject([
      { phase: 'started', callId: 'search-1', status: 'in_progress' },
      { phase: 'action', callId: 'search-1', query: 'current Node.js LTS', action },
      { phase: 'source', callId: 'search-1', sources: [source] },
      {
        phase: 'completed',
        status: 'completed',
  ...
  `);
    ...
    expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
      ...
      [emit] tool.result                         { "turnId": 0, "toolCallId": "call_lookup", "uiPreview": "lookup-result", "truncated": false, "output": "lookup-result" }
      [emit] agent.activity.updated              { "lifecycle": "ready", "turn": { "turnId": 0, "origin": { "kind": "user" }, "phase": "running", "step": 1, "ending": false, "pendingApprovals": [], "activeToolCalls": [], "since": "<time>" }, "background": [] }

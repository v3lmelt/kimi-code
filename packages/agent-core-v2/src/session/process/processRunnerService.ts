 ...
 * the actual host spawn to the App-scope `IHostProcessService`. Agent scopes
 * use a workspace-context runner so the cwd and isolation boundary follow the
 * Agent lease. A per-call `options.cwd` wins over the seeded cwd. A per-call
 * `options.env` is overlaid onto `process.env` and passed as the child's
 * complete env bag (the host replaces the child env with what we pass); when
 * `options.env` is omitted we pass `undefined` so the child inherits
...
export class AgentProcessRunner implements ISessionProcessRunner {
  declare readonly _serviceBrand: undefined;

  constructor(
    @ISessionWorkspaceContext private readonly ctx: ISessionWorkspaceContext,
    @IHostProcessService private readonly hostProcess: IHostProcessService,
  ) {}
  ...
  async exec(args: readonly string[], options?: ProcessExecOptions): Promise<IProcess> {
    ...
    if (command === undefined) {
      throw new BugIndicatingError(
        'AgentProcessRunner.exec(): at least one argument (the command to run) is required.',
      );
    }
    ...
    const cwd =
      requestedCwd === undefined
        ? this.ctx.workDir
        : this.ctx.isolation === undefined
          ? requestedCwd
          : this.ctx.assertAllowed(requestedCwd, 'execute');
    const env = this._buildExecEnv(options?.env);
    return this.hostProcess.spawn(command, args.slice(1), { cwd, env });
  }

  private _buildExecEnv(
    invocationEnv: Record<string, string> | undefined,
  ): Record<string, string> | undefined {
    if (invocationEnv === undefined) return undefined;
    return {
      ...(process.env as Record<string, string>),
      ...invocationEnv,
    };

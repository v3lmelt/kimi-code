/**
 * Scenario: resolveAuthBackedClient per-(provider, auth) client caching.
 * Responsibilities: identical per-request auth reuses the built SDK client
 * (no rebuild per turn), header order does not affect the signature, distinct
 * auth signatures and distinct provider instances build separately, failed
 * builds are never cached, and the clientFactory / constructor-cached-client
 * precedence is unchanged.
 * Run: pnpm exec vitest run packages/kosong/test/providers/request-auth.test.ts
 */
import type { ProviderRequestAuth } from '#/provider';
import { resolveAuthBackedClient } from '#/providers/request-auth';
